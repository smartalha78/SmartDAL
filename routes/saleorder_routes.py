...
@saleorder_bp.route('/get-saleorder-table-data', methods=['POST'])
@token_required
def get_saleorder_table_data():
    try:
        data = request.json
        if not data:
...
# ============= GET Customer =============
@saleorder_bp.route('/get-customers', methods=['GET'])
@token_required
def get_customers():
    try:
        customers = []
...
# ============= GET GODOWNS =============
@saleorder_bp.route('/get-godowns', methods=['GET'])
@token_required
def get_godowns():
    try:
        godowns = []
...
# ============= GET ITEMS =============
@saleorder_bp.route('/get-items', methods=['GET'])
@token_required
def get_items():
    try:
        items = []
...
# ============= GET UOMS =============
@saleorder_bp.route('/get-uoms', methods=['GET'])
@token_required
def get_uoms():
    try:
        uoms = []
...
@saleorder_bp.route('/get-saleorder-with-details', methods=['POST'])
@token_required
def get_saleorder_with_details():
    try:
        data = request.json
        if not data:
            return jsonify({"success": False, "error": "No JSON data provided"}), 400
        ...
        if not table_exists('invsaleorderhead') or not table_exists('invsaleorderdet'):
            return jsonify({
                "success": False, 
                "error": "SON tables not found in database"
            }), 404
        
        if vockey:
            head_query = "SELECT TOP 1 * FROM invsaleorderhead WHERE vockey = ? AND offcode = ?"
            head_result = execute_query(head_query, (vockey, offcode))
        else:
            head_query = "SELECT TOP 1 * FROM invsaleorderhead WHERE vno = ? AND offcode = ?"
            head_result = execute_query(head_query, (vno, offcode))
        
        if not head_result:
            return jsonify({"success": False, "error": "SON not found"}), 404
        
        head = head_result[0]
        actual_vockey = head.get('vockey')
        
        details_query = "SELECT TOP 100 * FROM invsaleorderdet WHERE vockey = ? AND offcode = ?"
...
# ============= INSERT SON =============
@saleorder_bp.route('/insert-saleorder-head-det', methods=['POST'])
@token_required
def insert_saleorder_head_det():
    try:
        data = request.json
        if not data:
            return jsonify({"success": False, "error": "No JSON data provided"}), 400
        ...
        vno = safe_str(f"SON{timestamp}", 15)
        ...
        head_columns = [
            ...
            'Ptype', 'CustCode', 'CustomerName', 'city', 'country',
            'godownid', 'godownname', 'currencycode', 'currencyrate',
            'compcode', 'createdby', 'createdate', 'status'
        ]
        
        head_values = [
            vno, vockey, company_offcode, "010101", vdate, "SON",
            head_data.get('Ptype', 110),
            safe_str(head_data.get('CustCode', ''), 20),
        ...
        head_query = f"INSERT INTO invsaleorderhead ({','.join(head_columns)}) VALUES ({placeholders})"
        ...
        for det in details:
            ...
            detail_query = f"INSERT INTO invsaleorderdet ({','.join(detail_columns)}) VALUES ({placeholders})"
...
# ============= UPDATE SON =============
@saleorder_bp.route('/update-saleorder-table-data', methods=['POST'])
@token_required
def update_saleorder_table_data():
    try:
        data = request.json
        if not data:
            return jsonify({"success": False, "error": "No JSON data provided"}), 400
        ...
        if not vno and not vockey:
            return insert_saleorder_head_det()
        ...
        try:
            ...
            if vockey:
                cursor.execute("DELETE FROM invsaleorderdet WHERE vockey = ?", (vockey,))
                cursor.execute("DELETE FROM invsaleorderhead WHERE vockey = ?", (vockey,))
            else:
                cursor.execute("DELETE FROM invsaleorderdet WHERE vno = ?", (vno,))
                cursor.execute("DELETE FROM invsaleorderhead WHERE vno = ?", (vno,))
...
# ============= GENERATE PDF =============
@saleorder_bp.route('/generate-saleorder-pdf', methods=['POST'])
@token_required
def generate_saleorder_pdf():
    try:
        data = request.json
        vno = data.get("vno")
        ...
        return jsonify({
            "success": True,
            "message": f"PDF generation for SON {vno} will be implemented",
            "pdfUrl": None
        }), 200
